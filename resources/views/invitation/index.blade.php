...
@section('content')
            ...
            <div class="card mb-4">
                ...
                <div class="card-header py-0 bg-primary">
                    <div class="d-flex align-items-center">
                        <h6 class="text-white">Custom Tamplates</h6>

                        <div class="ms-auto mt-3">
                            <a href="{{ route('invitationcard.create') }}"
                                    ...
                                    <div style="border-radius: 14px;">
                                        <div style="background-image: linear-gradient(transparent, transparent, transparent, transparent, transparent, #302d2d, black);border-radius: 17px;"
                                            ...
                                            <div class="mt-9">
                                                ...
                                                <div class="d-flex mt-2">
                                                    <div>
                                                        <a class="btn btn-icon-only btn-rounded btn-success mb-0 me-2 btn-sm d-flex align-items-center justify-content-center"
                                                            href="{{ secure_url('/invitationcard/' . $post->id . '/edit') }}">
                                                ...
                                                <form action="{{ url('invitationcard/' . $post->id) }}" method="POST"
                                                    id="form-{{ $post->id }}">
                                                    @method('DELETE')
                                                    @csrf
    ...
    <script>
        function cheakPremium(id) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ secure_url('/invitationcard-premium-action') }}",
                data: {
                    id: id,
                    type: cheak
                },
        ...
        $(".status-switch").change(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ secure_url('/invitationcard-status') }}",
                data: {
                    checked: checked,
                    id: id
                },
