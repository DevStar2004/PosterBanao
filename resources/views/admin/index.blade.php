                ...
                <div class="card-header py-0 bg-primary">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-auto mt-3">
                            <a href="{{ route('admins.create') }}" class="btn btn-success "><i class="fas fa-plus"></i> Add
                                New</a>
                        </div>
                    </div>
                </div>
                                    ...
                                    @foreach ($admins as $admin)
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <div class="form-switch align-items-center justify-content-center">
                                                    <input class="form-check-input status-switch"
                                                        @if ($admin->id == 1) disabled @endif type="checkbox"
                                                        data-id="{{ $admin->id }}"
                                                        @if ($admin->status == 0) checked @endif>
                                                </div>
                                            </td>
                                            ...
                                            <td class="align-middle text-center">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ date('d M, y', strtotime($admin->created_at)) }}</span>
                                            </td>
                                                ...
                                                <div class="ms-auto text-end align-middle text-center text-sm">
                                                    @if ($admin->id == 1)
                                                        @if (Session::get('userid') == '1')
                                                            <a href="#"
                                                                class="btn btn-icon-only btn-rounded btn-danger disabled"><i
                                                                    class="far fa-trash-alt"></i></a>
                                                            <a class="btn btn-icon-only btn-rounded btn btn-success"
                                                                href="{{ secure_url('/admins/' . $admin->id . '/edit') }}"><i
                                                                    class="fas fa-pencil-alt"></i></a>
                                                        @else
                                                            <a class="btn btn-icon-only btn-rounded btn-danger disabled"><i
                                                                    class="far fa-trash-alt"></i></a>
                                                            <a
                                                                class="btn btn-icon-only btn-rounded btn btn-success disabled"><i
                                                                    class="fas fa-pencil-alt"></i></a>
                                                        @endif
                                                    @else
                                                        <a href="#" data-id="{{ $admin->id }}" data-toggle="modal"
