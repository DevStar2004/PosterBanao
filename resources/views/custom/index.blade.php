...
@php
    $is_permitted = App\Models\Admin::isPermission('posts');
@endphp
                ...
                <div class="card-header py-0 bg-primary">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-auto mt-3">
                            <a href="{{ route('custom.create') }}" class="btn btn-success {{$is_permitted ? '' : 'disabled'}}">
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu" style="right:0;left:auto;">
                                    <li><a class="dropdown-item" href="#" data-type="enable" data-toggle="modal"
                                            data-target="#sectionModal">Add To Section</a></li>
                                    <li><a class="dropdown-item" href="#" data-type="enable" data-toggle="modal"
                                        ...
                                        <div style="background-image: linear-gradient(transparent, transparent, transparent, transparent, transparent, #302d2d, black);   border-radius: 17px;"
                                            ...
                                            @if ($post->section)
                                                <button class="badge badge-sm bg-gradient-dark position-absolute"
                                                    ...
                                                    {{ $post->section->name }} &nbsp; <i class="fas fa-window-close"></i>
                                                </button>
                                            @endif
                                            <div @if ($post->json != '') class="mt-8" @else class="mt-9" @endif>
                                                @if ($post->json != '')
                                                    <button class="btn btn-sm btn-primary btn_cust"
                                                ...
                                                <div class="d-flex mt-2">
                                                    <div>
                                                        <a class="btn btn-icon-only btn-rounded btn-success mb-0 me-2 btn-sm d-flex align-items-center justify-content-center"
                                                            href="{{ secure_url('/custom/' . $post->id . '/edit') }}">
                                                ...
                                                <form action="{{ url('custom/' . $post->id) }}" method="POST"
                                                    id="form-{{ $post->id }}">
                                                    @method('DELETE')
                                                    @csrf
