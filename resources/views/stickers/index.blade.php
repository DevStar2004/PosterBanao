                ...
                <div class="card-header py-0 bg-primary">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-auto mt-3">
                            <a href="{{ route('sticker.create') }}"
                                class="btn btn-success {{ $is_permitted ? '' : 'disabled' }}"><i class="fas fa-plus"></i> Add
                                New</a>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="form-group p-2">
                                <select class="form-control selectpicker" onchange="searchByCategoryID()" id="category">
                                    ...
                                    @foreach ($categories as $c)
                                        <option value="{{ $c->id }}"
                                            @if (!empty($category) && $c->name == $category) selected @endif>{{ $c->name }}</a>
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb">
                            @foreach ($stickers as $sticker)
                                <div class="col-xl-2 col-l-3 col-sm-4 p-2  bg-transparent">
                                    ...
                                    <div style="background:gray;background-image: url(@if ($sticker->item_url) {{ url($sticker->item_url) }} @else {{ url('/images/placeholder.jpg') }} @endif);height:140px;background-size: cover;"
                                        class="border-radius-xl">

                                        <div class="card-body position-relative z-index-1 p-2">

                                            <input type="checkbox" name="post_ids[]" value="{{ $sticker->id }}"
                                                ...
                                                <div class="d-flex mt-1">
                                                    <div>
                                                        <a class="btn btn-icon-only btn-rounded btn-success mb-0 me-2 btn-sm d-flex align-items-center justify-content-center"
                                                            href="{{ secure_url('/sticker/' . $sticker->id . '/edit') }}">
                                                    ...
                                                    <div>
                                                        <button
                                                            ...
                                                            data-id="{{ $sticker->id }}" data-toggle="modal"
                                                    ...
                                                    <div class="form-switch align-items-center justify-content-center">
                                                        <input class="form-check-input status-switch" type="checkbox"
                                                            data-id="{{ $sticker->id }}"
                                                            @if ($sticker->status == 0) checked @endif>
                                                    </div>

                                                </div>

                                                <form action="{{ url('sticker/' . $sticker->id) }}" method="POST"
                                                    id="form-{{ $sticker->id }}">
                                                    @method('DELETE')
                                                    @csrf
